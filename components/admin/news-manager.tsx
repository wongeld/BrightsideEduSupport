...
export default function AdminNewsManager() {
  ...
  const fetchNews = async () => {
    try {
      setLoading(true);
      const response = await fetch("/api/news?publishedOnly=false");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedNews) {
        newsData.append("id", formData.id);
      }

      newsData.append("title", formData.title);
      newsData.append("title_am", formData.title_am);
      newsData.append("content", formData.content);
      newsData.append("content_am", formData.content_am);
      newsData.append("excerpt", formData.excerpt);
      newsData.append("excerpt_am", formData.excerpt_am);
      newsData.append("published", formData.published.toString());

      // If we have a new image path from upload, use it
      if (imagePath) {
        newsData.append("image_path", imagePath);
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error ||
            `Failed to ${selectedNews ? "update" : "create"} news`
        );
      }
              ...
              <TabsContent value="am" className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={handleInputChange}
                    placeholder="Enter news title in Amharic"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={handleInputChange}
                    placeholder="Enter news content in Amharic"
                    rows={10}
                  />
                </div>
